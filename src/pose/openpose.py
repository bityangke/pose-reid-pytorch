...
def pose_si_processing(model, frame, pose_val, plot_skeleton):
    ...
    posebox, joint_coords_db, coord = cut_joint_image(oriImg, candidate, subset)


    """
    print(candidate.shape)
    print(subset.shape)
    posebox = np.zeros((people_num, LEN, LEN, 3))
    for index in range(people_num):
        posebox[i] = cut_joint_image(oriImg, subset, LEN)
    """
    return to_plot, posebox, joint_coords_db, coord
    # return oriImg, posebox, joint_coords_db, coord

"""
video_extract()
video_compose(img_folder='./extract_folder/')
"""

# pose_batch_processing()
